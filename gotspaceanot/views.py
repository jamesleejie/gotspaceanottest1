...
def filter(request):
    ...
    with connection.cursor() as cursor:
	    cursor.execute("SELECT student_department,COUNT(student_department) FROM NUS_system GROUP BY student_department GROUP BY matric_number HAVING matric_number = (SELECT matric_number FROM student)")
